...
/*
어노테이션을 통한 매핑. '*'를 통해 여러 요청명을 한번에 매핑처리한다.
즉, '.one'으로 끝나는 모든 요청명에 대해 매핑처리하였다.
★★★★★★★요청명은 반드시 슬러시(/)로 시작해야한다
 */
@WebServlet("*.one")
public class FrontController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		System.out.println(uri); ///K08JSPServlet/13Servlet/regist.one와 같은형태가 출력
		...
		if(commandStr.equals("/regist.one"))
			registFunc(req);//회원가입 처리할 메서드 호출
		else if(commandStr.equals("/login.one"))
			loginFunc(req);//로그인 처리할 메서드 호출
		else if(commandStr.equals("/freeboard.one"))
			freeboardFunc(req);//자유게시판 처리할 메서드 호출
