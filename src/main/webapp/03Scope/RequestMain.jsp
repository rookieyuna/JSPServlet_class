...
/*
request 영역
	: 클라이언트가 요청을 할 때 마다 새로운 request 객체가 생성되어,
...
<body>
	...
	<% 
		...
		request.removeAttribute("requestInteger"); //존재하지않는 정보지만 에러발생X
	%>
	<h2>request 영역의 속성값 읽기</h2>
	<% 
		//동일페에지에서 저장된 정보이므로 정상적으로 출력
		Person rPerson = (Person)(request.getAttribute("requestPerson"));
	%>
	<ul>
		<li>String 객체 : <%= request.getAttribute("requestString") %></li><!-- remove되어 null출력됨 -->
	...
	<%
	/*
	포워드 : 현재 페이지로 들어온 요청을 다음페이지로 보내는 기능을 말한다.
		즉 하나의 요청을 둘 이상의 페이지가 공유할 수 있다.
		포워드 명령을 만나기 전까지의 모든 내용은 버퍼에서 삭제되고,
		포워드 된 내용만 화면에 출력되는 특징이 있다. 
	*/
	request
