...
try{
	...
	//사용자의 웹브라우저 종류 파악을 위해 요청헤더 얻어오기
	String client = request.getHeader("User-Agent");
	if(client.indexOf("WOW64") == -1){//익스플로어가 아닌 경우 한글파일명 인코딩
		originalFilename = new String(originalFilename.getBytes("UTF-8"), "ISO-8859-1");
	}
	else{ //익스플로어인 경우 한글파일명 인코딩 방식
		originalFilename = new String(originalFilename.getBytes("KSC5601"), "ISO-8859-1");
	}
	/*
		위 처리를 통해 원본파일명이 한글인 경우 깨짐 방지
	...
	*/
	...
	1. 응답헤더 초기화
	2. 웹브라우저가 인식하지 못하는 컨텐츠 타입을 설정하여 다운로드 창을 강제로 띄움
		(jpg같은 확장자는 웹브라우저가 인식하여 파일을 바로 열기 때문)
