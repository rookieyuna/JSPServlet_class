...
<body>
	...
	<!-- 
		...
		해당 결과는 result에 true/false 로 저장
	 -->
	...
	<!-- 
		...
		통해 else와 같이 사용해야 한다.
		만약 여러개의 조건을 사용해야 할 경우라면 choose태그를 사용하는게 좋다.
	 -->
	...
	<c:if test="${ not result2 }"><!-- 앞 result의 not조건을 통해 else 구현 -->
		Java가 아닙니다. <br>
	</c:if>
	...
	<c:if test="${not(empty param.user or empty param.pass )}">
		...
		두번째 if문에서 not을 이용해서 else구문을 만들면된다.
		 -->
		<c:if test="${param.user == 'kosmo' && param.pass eq 1234 }" var="result">
			${ param.user }님 하이룽~<br>
		</c:if>
		<c:if test="${not result }">
			<span style="color:red;">아이디와 비번을 확인하세요</span>
		</c:if>
	</c:if>
