...
<body>
	...
	<!-- 
		Map컬렉션 사용 시 key값을 별도로 얻어올 필요 없이
	...
	for(String k : keys){
		//Key값을 토 ㅇ해 해당 객체를 얻어온다
		Person p = maps.get(k);
		out.println("Key=>" + k + "<br>");
		out.println("Value=> 이름:" + p.getName()+", "
					+ "나이:" + p.getAge()+"<br>");
	}
	/*
		JSTL의 확장for문을 사용하면 key값을 별도로 얻어오지 않으므로 훨씬 편리하다.
	*/
	%>
</body>
</html>